...
const ActionDropdown = ({ file }: { file: Models.Document }) => {
  ...
  const handleActions = async () => {
    ...
    const actions = {
      ...
      delete: () => deleteFileUsers({fileId: file.$id, bucketFileId: file.bucketFileId, path }),
    };
        ...
        {["rename", "delete", "share"].includes(value) && (
          <DialogFooter className="flex flex-col gap-3 md:flex-row">
            <Button
              ...
              className={"modal-cancel-button shadow-drop-1"}
            >
              Cancel
            </Button>
            <Button
              onClick={handleActions}
              className={"modal-submit-butto primary-btn"}
            >
